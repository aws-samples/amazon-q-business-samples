...
def create_slack_connector(
    ...
    For complete documentation on all available parameters and configuration options,
    see: https://docs.aws.amazon.com/amazonq/latest/qbusiness-ug/slack-api.html
    API Reference: https://docs.aws.amazon.com/amazonq/latest/api-reference/API_CreateDataSource.html
    
    Args:
        application_id (str): The Amazon Q Business application ID
        index_id (str): The Amazon Q Business index ID
        secret_arn (str): The ARN of the AWS Secrets Manager secret containing Slack token
        team_id (str): The Slack team ID from your Slack workspace URL
        data_source_name (str): Name for the data source (default: "Slack Data Source")
        since_date (str, optional): ISO 8601 date string to crawl from (default: 1 month ago)
        conversation_types (List[str], optional): Types of conversations to crawl 
                                                 (default: ["PUBLIC_CHANNEL", "PRIVATE_CHANNEL"])
        crawl_bot_messages (bool): Whether to crawl bot messages (default: False)
        exclude_archived (bool): Whether to exclude archived channels (default: True)
        max_file_size_mb (str): Maximum file size to crawl in MB (default: "50")
        sync_mode (str): Sync mode - "FULL_CRAWL", "FORCED_FULL_CRAWL", or "CHANGE_LOG"
        is_crawl_acl (bool): Whether to crawl access control information (default: True)
        ...
        document_enrichment_configuration (Dict[str, Any], optional): Document enrichment config
        media_extraction_configuration (Dict[str, Any], optional): Media extraction config
    ...
    # Reference: https://docs.aws.amazon.com/amazonq/latest/qbusiness-ug/slack-api.html
    configuration = {
        "type": "SLACK",
        "syncMode": sync_mode,
    ...
    if document_enrichment_configuration:
        create_data_source_params['documentEnrichmentConfiguration'] = document_enrichment_configuration
    
    if media_extraction_configuration:
        create_data_source_params['mediaExtractionConfiguration'] = media_extraction_configuration
    ...
    try:
        ...
        print(f"    🔧 Configuration size: {len(json.dumps(configuration))} characters")
        
        # Create the data source
        response = qbusiness_client.create_data_source(**create_data_source_params)
        
        print("💬 ✅ Slack data source created successfully!")
        print(f"    📊 Data Source ID: {response['dataSourceId']}")
        print(f"    📝 Data Source Name: {data_source_name}")
        print(f"    🏢 Team ID: {team_id}")
        print(f"    📅 Since Date: {configuration['additionalProperties']['sinceDate']}")
        ...
        if "InternalFailure" in str(e):
            ...
            print("    • Try again in a few minutes (could be temporary service issue)")
        ...
        print(f"🔍 Configuration being sent (sanitized):")
...
def create_slack_secret(
    ...
    Args:
        ...
        region_name (str, optional): AWS region name (uses default if not specified)
...
def create_iam_role_for_slack_connector(
    ...
    Based on AWS documentation: https://docs.aws.amazon.com/amazonq/latest/qbusiness-ug/slack-iam-role.html
    
    Args:
        role_name (str): Name for the IAM role
        application_id (str): Amazon Q Business application ID
        region_name (str, optional): AWS region name (auto-detected if not provided)
        ...
        "Statement": [
            {
                ...
                "Condition": {
                    ...
                    "ArnEquals": {
                        "aws:SourceArn": f"arn:aws:qbusiness:{region_name}:{account_id}:application/{application_id}"
                    }
                }
            }
        ]
    ...
    permissions_policy = {
        ...
        "Statement": [
            ...
            },
            {
                ...
                "Resource": [
                    f"arn:aws:kms:{region_name}:{account_id}:key/*"
                ],
                "Condition": {
                    "StringLike": {
                        "kms:ViaService": [
            ...
            },
            {
                ...
                "Resource": [
                    f"arn:aws:qbusiness:{region_name}:{account_id}:application/{application_id}",
                    f"arn:aws:qbusiness:{region_name}:{account_id}:application/{application_id}/index/*"
                ]
            },
            {
                ...
                "Resource": [
                    f"arn:aws:qbusiness:{region_name}:{account_id}:application/{application_id}",
                    f"arn:aws:qbusiness:{region_name}:{account_id}:application/{application_id}/index/*",
                    f"arn:aws:qbusiness:{region_name}:{account_id}:application/{application_id}/index/*/data-source/*"
                ]
            }
        ]
    }
    ...
    try:
        ...
        role_response = iam_client.create_role(
            ...
            Description=f"IAM role for Amazon Q Business Slack connector - Application {application_id}",
        ...
        policy_response = iam_client.create_policy(
            ...
            Description="Permissions policy for Amazon Q Business Slack connector"
        )
...
def setup_complete_slack_connector(
    ...
    """
    Complete setup: Create secret, IAM role, and Slack connector, optionally start sync.
    
    This is a convenience function that combines all required resource creation and connector setup.
    ...
    Args:
        ...
        sync_mode (str): Sync mode - "FULL_CRAWL", "FORCED_FULL_CRAWL", or "CHANGE_LOG"
        is_crawl_acl (bool): Whether to crawl access control information (default: True)
        role_arn (str, optional): IAM role ARN for the data source (will create if not provided)
        role_name (str, optional): Name for IAM role to create (default: auto-generated)
    ...
    Returns:
        Dict[str, Any]: Dictionary containing secret ARN, role ARN, data source response, and sync response
    """
    print("🚀 " + "="*60)
    ...
    if not validate_prerequisites(application_id, index_id, secret_arn):
        raise Exception("❌ Prerequisites validation failed. Please check the error messages above.")
