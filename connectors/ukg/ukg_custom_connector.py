...
# Configuration
SECRET_NAME = 'ukg-crawler-secrets'  # Change this to your AWS Secrets Manager secret name
...
def get_secret(secret_name: str) -> Dict[str, str]:
    ...
    The secret should be stored in AWS Secrets Manager as a JSON string with the following structure:
    {
    ...
    Args:
        secret_name: Name of the secret in Secrets Manager (e.g., 'ukg-crawler-secrets')
    ...
    Returns:
        ...
        - UKG_APPLICATION_SECRET: The UKG application secret for authentication
    ...
    Raises:
        ...
        Exception: If there's an error retrieving the secret from AWS Secrets Manager
    """
    try:
...
class UKGCrawler:
    def __init__(self, application_id: str, application_secret: str, client_id: str, base_url: str, s3_bucket: str):
        """
    ...
    def _get_oauth_token(self) -> Optional[str]:
        ...
        try:
            ...
            self.token_expiry = datetime.now() + timedelta(seconds=expires_in - 60)  # Subtract 1 minute for safety
    ...
    def _get_articles(self) -> list:
        ...
        while True:
            try:
                ...
                logger.info(f"Fetched {len(articles)} articles on this page")
    ...
    def _get_article_content(self, article_id: str) -> Optional[Dict[str, Any]]:
        """Fetch content body for a specific article and return the latest version."""
        try:
            response = requests.get(
                urljoin(self.base_url, f'/api/v2/client/kb_articles/{article_id}'),
                headers=self._get_headers()
            )
            response.raise_for_status()
            ...
            latest_version = max(
                versions,
                key=lambda v: datetime.fromisoformat(v.get('updated_at', '1970-01-01T00:00:00+00:00'))
            )
    ...
    def _create_metadata(self, article: Dict[str, Any], article_content: Dict[str, Any]) -> Dict[str, Any]:
        """Create metadata for Amazon Q Business ingestion."""
        article_id = article.get("id", "")
        source_uri = f"https://scif.employee.staging.us.people-doc.com/articles/{article_id}"
    ...
    def process_articles(self):
        ...
        for article in articles:
            ...
            if not article_content:
                logger.warning(f"Failed to fetch content for article {article_id}, skipping")
                continue
            ...
            successful_count += 1
            logger.info(f"Successfully processed article: {article_id} ({successful_count}/{len(articles)})")
        
        logger.info(f"Processing complete. Successfully processed {successful_count} out of {len(articles)} articles.")
    ...
    def _list_s3_articles(self) -> List[Dict[str, str]]:
        ...
        for page in paginator.paginate(Bucket=self.s3_bucket, Prefix='articles/'):
            ...
            for obj in page['Contents']:
                ...
                if key.endswith('.txt'):
                    ...
                    try:
                        ...
                        logger.warning(f"No metadata found for {key}, skipping")
                        continue
    ...
    def sync_with_q_business(self, application_id: str, index_id: str):
        ...
        logger.info(f"Found {len(articles)} articles to sync with Q Business")
        
        for article in articles:
            try:
                # Get content and metadata
                content = self._get_s3_object_content(article['content_key'])
                metadata_json = self._get_s3_object_content(article['metadata_key'])
                
                if not content or not metadata_json:
                    logger.warning(f"Missing content or metadata for {article['content_key']}, skipping")
                    continue
                ...
                document = {
                    'documentId': article['content_key'].replace('articles/', '').replace('.txt', ''),
                    'content': content,
                ...
                logger.info(f"Successfully synced article {document['documentId']} to Q Business")
                
            except Exception as e:
                logger.error(f"Error syncing article {article['content_key']} to Q Business: {e}")
                continue
...
def main():
    ...
    try:
        ...
        if not all([APPLICATION_ID, APPLICATION_SECRET, CLIENT_ID, BASE_URL, S3_BUCKET, 
                   Q_BUSINESS_APP_ID, Q_BUSINESS_INDEX_ID]):
        ...
        logger.error(f"Failed to initialize crawler while trying to seed secrets: {e}")
        return
